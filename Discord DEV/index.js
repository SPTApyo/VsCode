...
const { clientId, guildId, rappelsalonId } = require("./config.json");
...
const EMBED_FOOTER_TEXT =
  "IUTAssistant : \u004D\u0061\u0064\u0065\u0020\u0062\u0079\u0020\u005F\u0041\u0050\u0059\u004F";
const logDirectory = "./log";
const dataDirectory = "./data";
const logFileName = `${logDirectory}/log_${moment().format(
  "YYYY-MM-DD_HH-mm-ss"
)}.txt`;
...
if (!token) {
  logMessage(
    "\x1b[31m Erreur : le token n'est pas défini dans les variables d'environnement.\x1b[0m"
  );
  process.exit(1); // Arrête le bot si le token n'est pas défini
}

// Créer les dossiers s'ils n'existent pas
[logDirectory, dataDirectory].forEach(
...
function manageLogFiles() {
  logMessage(
    `\x1b[33m Vérification des fichiers de log excédentaire en cours ...\x1b[0m`
  );
  const logFiles = fs
    .readdirSync(logDirectory)
    .sort(
      (a, b) =>
        fs.statSync(`${logDirectory}/${b}`).mtime -
        fs.statSync(`${logDirectory}/${a}`).mtime
    )
  ...
  logFiles.forEach((file) => {
    ...
    logMessage(
      `\x1b[32m Suppression du fichier de log excédentaire : ${file}\x1b[0m`
    );
  });
}
...
const commands = [
  new SlashCommandBuilder()
    ...
    .addStringOption((option) =>
      option
        .setName("devoir")
        .setDescription("Le devoir à faire")
        .setRequired(true)
    )
    .addStringOption((option) =>
      option
        .setName("matiere")
        .setDescription("La matière du devoir")
        .setRequired(true)
    )
    .addStringOption((option) =>
      option
        .setName("date")
        .setDescription("La date limite (format: JJ-MM-AAAA)")
        .setRequired(true)
    ),
  new SlashCommandBuilder()
    .setName("modify")
    ...
    .addIntegerOption((option) =>
      option
        .setName("id")
        .setDescription("L'ID du devoir à modifier")
        .setRequired(true)
    )
    .addStringOption((option) =>
      option
        .setName("devoir")
        .setDescription("Le nouveau devoir (laisser vide pour ne pas modifier)")
    )
    .addStringOption((option) =>
      option
        .setName("matiere")
        .setDescription(
          "La nouvelle matière (laisser vide pour ne pas modifier)"
        )
    )
    .addStringOption((option) =>
      option
        .setName("date")
        .setDescription(
          "La nouvelle date limite (format: JJ-MM-AAAA, laisser vide pour ne pas modifier)"
        )
    ),
  new SlashCommandBuilder()
    .setName("list")
    .setDescription("Affiche la liste des devoirs ajoutés"),
  new SlashCommandBuilder()
    .setName("check")
    .setDescription("Vérifie les devoirs à venir et envoie des rappels"),
  new SlashCommandBuilder()
    .setName("sort")
    .setDescription("Trie du fichier JSON"),
  new SlashCommandBuilder()
    .setName("regenerate")
    .setDescription("Régénère les IDs de chaque devoir"),
  new SlashCommandBuilder()
    .setName("purge")
    .setDescription("Purge tous les devoirs enregistrés dans le fichier JSON"),
  new SlashCommandBuilder()
    .setName("about")
...
(async () => {
  try {
    ...
    logMessage(
      `\x1b[31m Erreur lors du déploiement des commandes : ${error}\x1b[0m`
    );
  }
})();
...
function removeExpiredDevoirs() {
  ...
  devoirs = devoirs.filter((d) =>
    moment(d.date, "DD-MM-YYYY").isSameOrAfter(now.subtract(1, "day"))
  );
...
function sortDevoirs(calledBy) {
  ...
  logMessage(
    `\x1b[32m Devoirs triés avec succès par la fonction : ${calledBy} !\x1b[0m`
  );
  saveDevoirs();
}
...
function hasRole(interaction, roleName, name) {
  ...
  logMessage(
    `\x1b[32m Vérifications des permissions d\'interaction pour ${name}\x1b[0m`
  );
...
client.on("interactionCreate", async (interaction) => {
  ...
  if (commandName === "add") {
    logMessage(
      `\x1b[34m Interaction : commande "ADD" par ${user.username}\x1b[0m`
    );
    ...
    if (
      ...
      logMessage(
        "\x1b[31m Erreur création de devoir : Devoir déjà existant\x1b[0m"
      );
    ...
    logMessage(
      `\x1b[34m Interaction : commande "MODIFY" par ${user.username}\x1b[0m`
    );
    ...
    if (!devoirToModify) {
      logMessage(
        `\x1b[31m Erreur modification de devoir : Devoir non trouvé avec l'ID ${id}\x1b[0m`
      );
    ...
    if (newDate && moment(newDate, "DD-MM-YYYY", true).isValid()) {
      ...
      logMessage(
        "\x1b[31m Erreur modification de devoir : Date invalide\x1b[0m"
      );
      return await interaction.reply(
        "Date invalide. Utilisez le format JJ-MM-AAAA."
      );
    ...
    logMessage(
      `\x1b[34m Interaction : commande "LIST" par ${user.username}\x1b[0m`
    );
    ...
    if (!hasRole(interaction, PERM_role, user.username)) {
  ...
  if (commandName === "regenerate") {
    ...
    logMessage(
      `\x1b[34m Interaction : commande "REGENERATEIDS" par ${user.username}\x1b[0m`
    );
    logMessage("\x1b[33m Régénération des IDs de devoirs en cours ...\x1b[0m");
    regenerateIds();
    await interaction.reply(
      "Les identifiants de devoirs ont été régénérés avec succès !"
    );
    logMessage(
      "\x1b[32m Les identifiants de devoirs ont été régénérés avec succès !\x1b[0m"
    );
    saveDevoirs();
    ...
    logMessage(
      `\x1b[34m Interaction : commande "PURGE" par ${user.username}\x1b[0m`
    );
    ...
    logMessage(
      `\x1b[34m Interaction : commande "ABOUT" par ${user.username}\x1b[0m`
    );
    await interaction.reply(
      `Name : ${name}\nAuthor : ${author}\nVersion : ${Appversion}\nLicense : ${license}`
    );
...
async function checkForUpcomingDevoirs(interaction) {
  ...
  const reminders = devoirs.filter((d) => {
    ...
    return [7, 6, 5, 4, 3, 2, 1, 0, -1].includes(
      dueDate.diff(now.clone().startOf("day"), "days")
    );
  });
  ...
  if (channel) {
    if (reminders.length > 0) {
      try {
        ...
        logMessage(
          `\x1b[32m ${fetched.size} messages ont été supprimés dans le salon (${channel.name}) avec succès !\x1b[0m`
        );
      } catch (error) {
        logMessage(
          "\x1b[31m Erreur lors de la suppression des messages : \x1b[0m",
          error
        );
      }
    }
  ...
  if (interaction) {
    if (!interaction.replied) {
      ...
      logMessage(
        reminders.length
          ? "\x1b[32m Rappels de devoir envoyés avec succès !\x1b[0m"
          : "\x1b[32m Aucun devoir à rappeler.\x1b[0m"
      );
    } else {
    ...
    logMessage(
      reminders.length
        ? "\x1b[32m Rappels automatiques envoyés avec succès !\x1b[0m"
        : "\x1b[32m Aucun rappel automatique à envoyer\x1b[0m"
    );
  }
}
...
async function loginWithRetry() {
  while (true) {
    try {
      ...
      logMessage(
        `\x1b[31m Erreur de connexion, réessai dans 5 secondes... ${error} \x1b[0m`
      );
