...
int counting=0;
    ...
    logs();
    fprintf(inputfile,"%s:",heure);
    fputs("R�ponse utilisateur:", inputfile );

    if ((Limit=='Y')||(Limit=='y')&&(numero==limite))
    {
    WAIT();
    printf("R%cponse autonome...\n",130);
    fputs(" �R�ponse autonome�...\n\n", inputfile );
    sleep(2);
    }
    else
    {
   ...
   fputs("[ERREUR]>Fichier test manquant.. \n", inputfile );
    ...
    logs();
    fprintf(inputfile,"%s:",heure);
    fputs("[OK]>S�quence termin�e int�gralit� des fichiers supprimer avec succ�s \n", inputfile );
    Limit='y';
    limite=numero;
    DEL=NULL;
    while((DEL!='y')||(DEL!='Y'))
    ...
    logs();
    fprintf(inputfile,"%s:",heure);
    fprintf(inputfile,"R�ponse utilisateur: �%c�...", DEL );
    if ((DEL=='y')||(DEL=='Y'))
    {
        sleep(2);
        logs();
        ...
        fprintf(inputfile,"[SETTING]>ACCESS:<<%s>>\n",chemin);
           ...
           if (counting==numero)
   {
   fputs("[SUMMARY]>Test complet aucune d�faillance d�tect�e.. \n", inputfile );
   fprintf(inputfile,"[SUMMARY]>Fichier pr�sent: %d/%d\n",counting, numero );
   }
   else
{
    ...
    fprintf(inputfile,"[SUMMARY]>int�grit� des donn�es estim�e a %d %%\n\n",pourc);
